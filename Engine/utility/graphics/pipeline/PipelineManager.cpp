...
Microsoft::WRL::ComPtr<ID3D12RootSignature> PipelineManager::CreateDeferredLightingRootSignature()
{
    ...
    // テクスチャ系（t0〜t5）はディスクリプタテーブル、
    // StructuredBuffer（t6, t7）はルートSRVで直接渡す。
    ...
    // t0〜t5: アルベド / 法線 / マテリアル / 深度 / シャドウマップ / 環境キューブ
    for (int i = 0; i < 6; ++i)
    {
        rootParameters[4 + i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
        rootParameters[4 + i].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
        rootParameters[4 + i].DescriptorTable.pDescriptorRanges = &ranges[i];
        rootParameters[4 + i].DescriptorTable.NumDescriptorRanges = 1;
    }
    // t6: ポイントライト配列
    ...
    // s0: 通常サンプラー（環境キューブ用） / s1: シャドウ比較サンプラー
    D3D12_STATIC_SAMPLER_DESC staticSamplers[2] = {};
    staticSamplers[0] = CreateCommonSamplerDesc();
...
Microsoft::WRL::ComPtr<ID3D12RootSignature> PipelineManager::CreateLine3dRootSignature()
{
    ...
    // b0: ビュープロジェクション行列（フレーム中は不変なので1回だけ設定する）
    rootParameters[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
    ...
    rootParameters[1].Constants.Num32BitValues = 20; // float4x4(16) + float4(4)
    descriptionRootSignature.pParameters = rootParameters;             // ルートパラメータ配列へのポインタ
    descriptionRootSignature.NumParameters = _countof(rootParameters); // 配列の長さ

    // サンプラーは使わない（テクスチャを一切参照しないため）
