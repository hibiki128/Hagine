...
void TestCollisionManager::Register(ColliderBase *collider) {
    if (!collider)
        return;
...
void TestCollisionManager::Unregister(ColliderBase *collider) {
    if (!collider)
        return;
...
void TestCollisionManager::UpdateColliderTag(ColliderBase *collider, const std::string &oldTag, const std::string &newTag) {
    if (!collider)
        return;
...
void TestCollisionManager::Clear() {
    collidersByTag_.clear();
    collisionStates_.clear();
}

void TestCollisionManager::Update() {
    UpdateColliders();
    CheckCollisions();
}

void TestCollisionManager::UpdateColliders() {
...
void TestCollisionManager::CheckCollisions() {
...
void TestCollisionManager::CheckCollisionPair(ColliderBase *a, ColliderBase *b) {
...
bool TestCollisionManager::TestCollision(ColliderBase *a, ColliderBase *b) {
...
void TestCollisionManager::DebugDraw(const ViewProjection &viewProjection) {
    for (auto &[tag, colliders] : collidersByTag_) {
        for (auto *collider : colliders) {
...
bool TestCollisionManager::IsCollision(const Sphere &s1, const Sphere &s2) {
...
bool TestCollisionManager::IsCollision(const AABB &aabb1, const AABB &aabb2) {
    return (aabb1.min.x <= aabb2.max.x && aabb1.max.x >= aabb2.min.x) &&
           (aabb1.min.y <= aabb2.max.y && aabb1.max.y >= aabb2.min.y) &&
           (aabb1.min.z <= aabb2.max.z && aabb1.max.z >= aabb2.min.z);
}

bool TestCollisionManager::IsCollision(const OBB &obb1, const OBB &obb2) {
    Vector3 axes[15] = {
        obb1.orientations[0],
        obb1.orientations[1],
        obb1.orientations[2],
...
bool TestCollisionManager::IsCollision(const AABB &aabb, const Sphere &sphere) {
...
bool TestCollisionManager::IsCollision(const OBB &obb, const Sphere &sphere) {
...
bool TestCollisionManager::IsCollision(const AABB &aabb, const OBB &obb) {
...
void TestCollisionManager::ProjectOBB(const OBB &obb, const Vector3 &axis, float &min, float &max) {
...
void TestCollisionManager::ProjectAABB(const Vector3 &axis, const AABB &aabb, float &outMin, float &outMax) {
    Vector3 vertices[8];
    vertices[0] = aabb.min;
...
bool TestCollisionManager::TestAxis(const Vector3 &axis, const OBB &obb1, const OBB &obb2) {
...
bool TestCollisionManager::TestAxis(const Vector3 &axis, const AABB &aabb, const OBB &obb) {
