...
void EnemyUI::Update()
{
    ...
    {
        // HP割合を計算
        float hpRatio = static_cast<float>(pEnemy_->GetHP()) / static_cast<float>(pEnemy_->GetMaxHP());
        float energyRatio = static_cast<float>(pEnemy_->GetEnergy()) / static_cast<float>(pEnemy_->GetMaxEnergy());
        // HPバーのサイズを割合に応じて調整
