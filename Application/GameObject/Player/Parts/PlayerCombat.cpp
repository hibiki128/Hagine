...
void PlayerCombat::StartSkillStaging() {
    ...
    skillCutscene_.Start(owner_, owner_->GetCamera(), [this] {
        makanAttack_ptr_->Activate(owner_->GetTransformPtr());
        owner_->EmitAction(Player::ActionKind::Special); // 入力表示UI用：必殺技を通知
    });
}
