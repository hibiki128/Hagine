                        ...
                        const Vector3 &forward, bool active, bool grounded)
{
    ...
    if (pWindEmitter_)
    {
        ...
        pWindEmitter_->SetTranslate(center + dir * kWindForwardOffset);
        Vector3 back = -dir;
        Vector3 spread = {kWindSpread, kWindSpread, kWindSpread};
        pWindEmitter_->SetMinVelocity(back * kWindSpeedMin - spread);
        pWindEmitter_->SetMaxVelocity(back * kWindSpeedMax + spread);

        // 地上ダッシュはエミッターを縦に伸ばし、空中ダッシュは基準スケールに戻す
        Vector3 scale = baseWindScale_;
        scale.y = grounded ? kGroundWindScaleY : baseWindScale_.y;
        pWindEmitter_->SetScale(scale);

        pWindEmitter_->SetAuto(active);
    }
}
