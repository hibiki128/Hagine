...
void FollowCamera::StartSkillCloseUp(BaseObject *performer) {
    skillCloseUpTarget_ = performer;
}

void FollowCamera::EndSkillCloseUp() {
    // 対象を外すだけで、次フレームからは通常の追従計算が絶対座標で走るため
    // 補間なしで「パッと」通常カメラへ戻る
    skillCloseUpTarget_ = nullptr;
}

bool FollowCamera::UpdateSkillCloseUp() {
    ...
    worldTransform_.translation_ += (goalPos - worldTransform_.translation_) * t;

    // 常に顔を注視する
    Vector3 look = facePos - worldTransform_.translation_;
    if (look.Length() > kEpsilon) {
        ...
        worldTransform_.quateRotation_ = Quaternion::FromMatrix(MakeRotateMatrix(right, up, look));
    }
