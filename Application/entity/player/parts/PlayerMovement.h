...
class PlayerMovement
{
  public:
    ...
    /// <summary>
    /// ゲームパッドの A トリガーでダッシュを開始できるなら開始する。
    /// 「A → 少し遅れてスティック」の操作を静止（Idle）状態からでも受け付けられるよう、
    /// Move() だけでなく Idle 系ステートからも毎フレーム呼べる共通処理として切り出す。
    /// A 押下時にスティックがニュートラルなら猶予時間を与え、その間に倒せばダッシュ継続する。
    /// </summary>
    /// <returns>この呼び出しでダッシュを開始したら true</returns>
    bool TryStartGamepadDash();
