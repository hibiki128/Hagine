...
bool BehaviorTreeEditor::IsInputPin(ed::PinId p) { return (static_cast<int>(p.Get()) - kPinOffset) % 10 == 1; }
bool BehaviorTreeEditor::IsOutputPin(ed::PinId p) { return (static_cast<int>(p.Get()) - kPinOffset) % 10 == 2; }
bool BehaviorTreeEditor::IsSuccessPin(ed::PinId p) { return (static_cast<int>(p.Get()) - kPinOffset) % 10 == 3; }
bool BehaviorTreeEditor::IsFailurePin(ed::PinId p) { return (static_cast<int>(p.Get()) - kPinOffset) % 10 == 4; }
    ...
    for (auto &node : nodes_)
    {
        if (node.IsWeightNode())
        {
            for (int i = 0; i < static_cast<int>(node.WeightedOutputs.size()); ++i)
            {
                ...
                {
                    outNodeId = static_cast<int>(node.ID.Get());
                    outOutputIndex = i;
                    return true;
                }
            }
...
void BehaviorTreeEditor::BuildAndRunTree()
{
    ...
    if (runtimeRoot_)
    {
        runtimeRoot_->SetContext(pDebugEnemy_, pDebugPlayer_);
        if (pDebugEnemy_)
            pDebugEnemy_->SetBehaviorTree(runtimeRoot_);
    }
}
...
std::shared_ptr<BTNode> BehaviorTreeEditor::BuildNodeRecursive(int editorNodeId)
{
    auto it = std::find_if(nodes_.begin(), nodes_.end(),
                           [editorNodeId](const EditorNode &n) { return static_cast<int>(n.ID.Get()) == editorNodeId; });
    if (it == nodes_.end())
        return nullptr;
    ...
    if (!isLeaf)
    {
        ...
        {
            int outputPinId = static_cast<int>(eNode.OutputPinID.Get());
    ...
    {
        int successPinId = static_cast<int>(eNode.SuccessPinID.Get());
        int failurePinId = static_cast<int>(eNode.FailurePinID.Get());
...
std::vector<int> BehaviorTreeEditor::FindChildrenNodeIds(int outputPinId)
{
    ...
    {
        if (static_cast<int>(link.StartPinID.Get()) == outputPinId)
        {
            int endPin = static_cast<int>(link.EndPinID.Get());
...
std::vector<std::pair<int, float>> BehaviorTreeEditor::FindWeightedChildrenNodeIds(const EditorNode &node)
{
    ...
    for (int i = 0; i < static_cast<int>(node.WeightedOutputs.size()); ++i)
    {
        int outputPinId = static_cast<int>(node.WeightedOutputs[i].PinID.Get());
        float weight = node.WeightedOutputs[i].Weight;
        for (const auto &link : links_)
        {
            if (static_cast<int>(link.StartPinID.Get()) == outputPinId)
            {
                int childNodeId = (static_cast<int>(link.EndPinID.Get()) - kPinOffset - 1) / 10;
...
int BehaviorTreeEditor::FindRootNodeId()
{
    ...
    {
        int inputPin = static_cast<int>(node.InputPinID.Get());
        bool hasInput = false;
        for (const auto &link : links_)
        {
            if (static_cast<int>(link.EndPinID.Get()) == inputPin)
            {
                hasInput = true;
                break;
            }
        ...
        if (!hasInput)
            return static_cast<int>(node.ID.Get());
    }
    return -1;
}
...
void BehaviorTreeEditor::SaveTree()
{
    ...
    {
        json n;
        n["id"] = static_cast<int>(node.ID.Get());
        n["title"] = node.Title;
        ...
        if (node.IsWeightNode())
        {
            ...
            {
                json w;
                w["pinId"] = static_cast<int>(output.PinID.Get());
    ...
    {
        json l;
        l["id"] = static_cast<int>(link.ID.Get());
        l["start"] = static_cast<int>(link.StartPinID.Get());
        l["end"] = static_cast<int>(link.EndPinID.Get());
    ...
    for (const auto &n : nodesJson)
    {
        ...
        if (node.IsWeightNode() && n.contains("weightedOutputs"))
        {
            ...
            for (const auto &w : n["weightedOutputs"])
            {
                ...
                if (static_cast<int>(output.PinID.Get()) > maxPinId)
                    maxPinId = static_cast<int>(output.PinID.Get());
            }
        }
...
void BehaviorTreeEditor::OnImGuiRender()
{
    ...
    ImGui::SameLine();
    {
        ...
        int selNodeId = (selCount > 0) ? static_cast<int>(selectedNodes[0].Get()) : -1;

        // 選択ノード名を表示
        const char *selName = "---";
        for (const auto &n : nodes_)
        {
            if (static_cast<int>(n.ID.Get()) == selNodeId)
            {
        ...
        {
            bool canTest = (selNodeId != -1) && (pDebugEnemy_ != nullptr);
            if (!canTest)
                ImGui::BeginDisabled();
            if (ImGui::Button(" ▷ 単体テスト "))
            {
                ...
                if (singleNode)
                {
                    singleNode->SetContext(pDebugEnemy_, pDebugPlayer_);
                    singleNode->Reset();
...
void BehaviorTreeEditor::DrawNodeParameters(EditorNode &node)
{
    ...
    case EditorNodeType::DecoratorWeight: {
        ...
        if (removeIndex >= 0)
        {
            ...
            const int removedPin = static_cast<int>(node.WeightedOutputs[removeIndex].PinID.Get());
            links_.erase(std::remove_if(links_.begin(), links_.end(),
                                        [removedPin](const EditorLink &l) { return static_cast<int>(l.StartPinID.Get()) == removedPin; }),
                         links_.end());
    ...
    case EditorNodeType::ActionComboFull: {
        ImGui::DragFloat("1段の時間(s)", &node.Parameter, 0.02f, 0.0f, 10.0f, "%.2f");
        int steps = static_cast<int>(node.Parameter2);
        if (ImGui::DragInt("最大段数", &steps, 0.1f, 0, 20))
            node.Parameter2 = static_cast<float>(steps);
    ...
    case EditorNodeType::ActionBurstShoot: {
        ...
        int cnt = static_cast<int>(node.Parameter2);
        if (ImGui::DragInt("発射弾数", &cnt, 0.1f, 1, 100))
            node.Parameter2 = static_cast<float>(cnt);
    ...
    if (ed::BeginDelete())
    {
        ...
        while (ed::QueryDeletedNode(&nodeId))
        {
            if (ed::AcceptDeletedItem())
            {
                ...
                auto it = std::find_if(nodes_.begin(), nodes_.end(), [id](const EditorNode &n) { return static_cast<int>(n.ID.Get()) == id; });
                if (it != nodes_.end())
                {
        ...
        while (ed::QueryDeletedLink(&linkId))
        {
            if (ed::AcceptDeletedItem())
            {
                int id = static_cast<int>(linkId.Get());
                links_.erase(std::remove_if(links_.begin(), links_.end(), [id](const EditorLink &l) { return static_cast<int>(l.ID.Get()) == id; }), links_.end());
