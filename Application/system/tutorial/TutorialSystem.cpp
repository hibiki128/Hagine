...
void TutorialSystem::Initialize(Player *player)
{
    ...
    prevStateName_ = pPlayer_->GetCurrentStateName();
...
void TutorialSystem::Update(float dt)
{
    ...
    prevStateName_ = pPlayer_->GetCurrentStateName();
    wasGroundedLastFrame_ = pPlayer_->GetIsGrounded();
}
...
bool TutorialSystem::CheckMove()
{
    ...
    const std::string &state = pPlayer_->GetCurrentStateName();
...
bool TutorialSystem::CheckJump()
{
    ...
    const std::string &current = pPlayer_->GetCurrentStateName();
...
bool TutorialSystem::CheckDescend()
{
    const std::string &state = pPlayer_->GetCurrentStateName();
    bool inFly = (state == "FlyIdle" || state == "FlyMove");
    bool isGrounded = pPlayer_->GetIsGrounded();
...
bool TutorialSystem::CheckAirMove()
{
    ...
    return (pPlayer_->GetCurrentStateName() == "FlyMove");
}

bool TutorialSystem::CheckDash()
{
    // ダッシュが開始したフレームを検出してカウント
    return pPlayer_->GetDashStartedThisFrame();
}
...
bool TutorialSystem::CheckLanding()
{
    ...
    bool justLanded = (pPlayer_->GetIsGrounded() && !wasGroundedLastFrame_);
...
bool TutorialSystem::CheckChargeAttack(float dt)
{
    ...
    GamePad *pad = pPlayer_->GetGamePad();
    ...
    if (!pad->IsConnected())
    {
        isHolding = pInput_->PushKey(DIK_J);
        isReleased = pInput_->ReleaseMomentKey(DIK_J);
    }
    else
    {
    ...
    {
        bool isChargeShot = (chargeHoldTimer_ >= pPlayer_->GetChargeThreshold());
        chargeHoldTimer_ = 0.0f;
        chargeInputActive_ = false;
        return isChargeShot;
    }
...
bool TutorialSystem::CheckEnergyCharge()
{
    ...
    return (pPlayer_->GetCurrentStateName() == "EnergyCharge");
}

bool TutorialSystem::CheckSpecialAttack()
{
    return pPlayer_->GetIsSkillActive();
}
...
bool TutorialSystem::IsMoveInput() const
{
    ...
    if (!pad->IsConnected())
    {
        return pInput_->PushKey(DIK_W) || pInput_->PushKey(DIK_A) ||
               pInput_->PushKey(DIK_S) || pInput_->PushKey(DIK_D);
    }
...
bool TutorialSystem::IsAscendInput() const
{
    ...
    if (!pad->IsConnected())
    {
        return pInput_->PushKey(DIK_SPACE);
    }
...
bool TutorialSystem::IsDescendInput() const
{
    ...
    if (!pad->IsConnected())
    {
        return pInput_->PushKey(DIK_LSHIFT);
    }
...
bool TutorialSystem::IsMeleeInput() const
{
    ...
    if (!pad->IsConnected())
    {
        return pInput_->TriggerKey(DIK_H);
    }
...
bool TutorialSystem::IsRangedTrigger() const
{
    ...
    if (!pad->IsConnected())
    {
        return pInput_->TriggerKey(DIK_J);
    }
